#ifndef DLIB_DNN_SV_INPUT_ARRAY_H_
#define DLIB_DNN_SV_INPUT_ARRAY_H_
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        friend void serialize(const input_sv_array& item, std::ostream& out)
        {
            serialize("input_sv_array", out);
            for (size_t k = 0; k < K; ++k)
                serialize(item.avg_color[k], out);
            serialize(item.scale, out);
        }

        friend void deserialize(input_sv_array& item, std::istream& in)
        {
            std::string version;
            deserialize(version, in);
            if (version != "input_sv_array")
                throw serialization_error("Unexpected version found while deserializing dlib::input_sv_array.");
            for (size_t k = 0; k < K; ++k)
                deserialize(item.avg_color[k], in);	
            deserialize(item.scale, in);
        }

        friend std::ostream& operator<<(std::ostream& out, const input_sv_array& item)
        {
            out << "input_sv_array(";
            std::string tmp = "";
        ...
        friend void to_xml(const input_sv_array& item, std::ostream& out)
        {
            out << "<input_sv_array ";			
            for (size_t k = 0; k < K-1; ++k)
...
#endif	// DLIB_DNN_SV_INPUT_ARRAY_H_
