...
std::string version;
std::string net_name = "sv_net_";
std::string net_sync_name = "sv_sync_";
std::string log_filename = "sv_net_";
...
void get_platform_control(void)
{
    ...
    log_filename = log_filename + version + "_";
...
void print_usage(void)
{
    ...
    std::cout << "sv_dnn <input_file.txt>" << std::endl;
    std::cout << endl;
}
...
int main(int argc, char** argv)
{
    ...
    parse_input_data_file(input_filename, version, stop_criteria, tp, train_inputfile, test_inputfile, ci, avg_color, filter_num);
...
//-----------------------------------------------------------------------------
        ...
        uint32_t num_test_images = te.size();
...
*/
        ...
        for (idx = 0; idx < num_test_images; ++idx)
        {
            ...
            var_gt_accum += tmp_results(0, 3);
            var_dm_accum += tmp_results(0, 4);
